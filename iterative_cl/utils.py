...
def get_log_image_output_folder(log_folder: Path) -> Path:

    now = datetime.datetime.now()

    return log_folder.joinpath(f"image_output_folder_{now.month}{now.day}{now.year}"
                               f"_{now.hour}{now.minute}{now.second}")

def get_log_csv_class_distribution(log_folder: Path) -> Path:

    now = datetime.datetime.now()

    return log_folder.joinpath(f"class_distribution_{now.month}{now.day}{now.year}"
                               f"_{now.hour}{now.minute}{now.second}.csv")

def get_log_csv_magnitude_medians(log_folder: Path) -> Path:

    now = datetime.datetime.now()

    return log_folder.joinpath(f"magnitude_medians_{now.month}{now.day}{now.year}"
                               f"_{now.hour}{now.minute}{now.second}.csv")
